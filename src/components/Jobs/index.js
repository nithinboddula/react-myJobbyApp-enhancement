  ...
  changeEmployeeList = (type, isChecked) => {
    ...
    if (isChecked) {
      if (employeeType.includes(type)) {
        newEmployeeList = [...employeeType]
      } else {
        newEmployeeList = [...employeeType, type]
      }
    } else if (employeeType.includes(type)) {
      newEmployeeList = employeeType.filter(eachType => eachType !== type)
    }

    this.setState({employeeType: [...newEmployeeList]}, this.getJobs)
  }
